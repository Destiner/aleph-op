...
function useMethods(): UseMethods {
  ...
  const defaults = computed(() => getDefaults(id.value));
  const methods = computed(() => getMethodList(id.value, defaults.value));
...
function getDefaults(chain: Chain | null): Defaults {
  ...
  // TODO use real contract addresses
  const EMPTY_ADDRESS = '0x0000000000000000000000000000000000000000';
  const contractMap: Record<Chain, string> = {
    [ETHEREUM]: '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2',
    [GOERLI]: EMPTY_ADDRESS,
    [OPTIMISM]: EMPTY_ADDRESS,
    [OPTIMISM_GOERLI]: EMPTY_ADDRESS,
    [BASE]: EMPTY_ADDRESS,
...
function getMethodList(id: Chain, defaults: Defaults): Method[] {
  const { blockHash, transactionHash, transactionInput, address, contract } =
    defaults;

  return [
    {
      id: OPTIMISM_OUTPUT_AT_BLOCK,
      name: 'Get output at block',
      type: 'optimism',
    ...
    {
      id: OPTIMISM_ROLLUP_CONFIG,
      name: 'Get rollup config',
      type: 'optimism',
      description: 'Returns the rollup configuration parameters.',
      params: [],
    },
    {
      id: OPTIMISM_VERSION,
